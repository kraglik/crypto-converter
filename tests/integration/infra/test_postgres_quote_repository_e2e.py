...
@pytest.mark.asyncio
async def test_postgres_writer_and_repository_roundtrip(session_factory: async_sessionmaker[AsyncSession]):
    # Given
    rate_factory = RateFactory(PrecisionService())
    writer = PostgresQuoteWriter(session_factory=session_factory, rate_factory=rate_factory)
    repo = PostgresQuoteRepository(session_factory=session_factory, rate_factory=rate_factory)
    ...
    before = await repo.get_latest_before(q2.pair, TimestampUTC(base_time - timedelta(seconds=20)))
