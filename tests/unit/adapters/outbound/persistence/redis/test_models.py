...
def test_redis_ticker_happy_path_and_roundtrip():
    # Given
    t = RedisTicker(symbol="BTCUSDT", rate=Decimal("25000.5"), timestamp=datetime(2025, 10, 2, 0, 0, tzinfo=timezone.utc))
...
def test_redis_ticker_validation():
    ...
    with pytest.raises(ValueError):
        RedisTicker(symbol="btcusdt", rate=Decimal("1"), timestamp=datetime.now(timezone.utc))
    with pytest.raises(ValueError):
        RedisTicker(symbol="BTCUSDT", rate=Decimal("-1"), timestamp=datetime.now(timezone.utc))
...
def test_redis_ticker_from_dict_errors():
    with pytest.raises(ValueError):
        RedisTicker.from_dict({"symbol": "BTCUSDT", "timestamp": "2025-10-02T00:00:00+00:00"})
    with pytest.raises(ValueError):
        RedisTicker.from_dict({"symbol": "BTCUSDT", "rate": "x", "timestamp": "2025-10-02T00:00:00+00:00"})